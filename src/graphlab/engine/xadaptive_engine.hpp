...
namespace graphlab
{
    ...
    public:
        ...
        void x_inner_signal_vset(const std::string& order = "shuffle")
        {
            ...
            foreach(lvid_type lvid, vtxs)
            {
                ...
                double priority;
                xmessages.light_add(lvid, messages[lvid], &priority);
        ...
        execution_status::status_enum xstart()
        {
            ...
            if((rmi.procid()==0)&&(!first_time_start))
                logstream(LOG_INFO)<<rmi.procid()<<"-"<<fiber_control::get_worker_id()
                                   <<"  SYNC->ASYNC, async start at "<<globaltimer.current_time_millis()<<std::endl;
    ...
    */
    ...
    num_updates() const
    {
        return completed_applys.value+programs_executed.value;
    }
    ...
    {
        return timer::approx_time_seconds() - start_time;
    }
    ...
    xadaptive_engine<VertexProgram>::sstart()
    {
        ...
        if (rmi.procid() == 0)
        {
            logstream(LOG_EMPH) << rmi.procid()<<": Iteration counter will only output every 5 seconds. Start at "
                                << globaltimer.current_time_millis() << std::endl;
        }
                ...
                else
                {
                    ...
                    if(lastactive!=prelastactive)
                    {
                        ...
                        if(tmpc<0)
                        {
                            ...
                            if(tmpk2<tmpk)
                            {
                                tmpc = (this_iter_time-tmpk2*lastactive)/2;
                                tmpk = tmpk2;
                            }
                            else
                            {
        ...
        if (rmi.procid() == 0)
        {
            logstream(LOG_EMPH) << iteration_counter
                                << " iterations completed, tol computation "<<total_act<< std::endl;
    ...
    xadaptive_engine<VertexProgram>::start()
    {
        ...
        while(termination_reason==execution_status::MODE_SWITCH)
        {
            ...
            if(current_engine == X_SYNC)
            {
                ...
                if (rmi.procid() == 0)
                    logstream(LOG_EMPH) << "Switch to ASYNC. At "<<(globaltimer.current_time_millis())<<std::endl;
                termination_reason = xstart();
            }
            else
            {
                //switch to SYNC
                if (rmi.procid() == 0)
                    logstream(LOG_EMPH) << "Switch to SYNC. At "<<(globaltimer.current_time_millis())<<std::endl;
                force_abort = false;
                iteration_counter = 0;
                completed_applys = 0;
                has_message.clear();
