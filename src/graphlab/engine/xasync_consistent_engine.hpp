...
namespace graphlab {
  ...
  class xasync_consistent_engine: public iengine<VertexProgram> {
  ...
  private:
     ...
     */
    typedef context<xasync_consistent_engine> context_type;

    // context needs access to internal functions
    friend class context<xasync_consistent_engine>;
    ...
    typedef xasync_consistent_engine<VertexProgram> engine_type;

    typedef conditional_addition_wrapper<gather_type> conditional_gather_type;
    
    /// The RPC interface
    dc_dist_object<xasync_consistent_engine<VertexProgram> > rmi;
  ...
  public:
     ...
     */
    xasync_consistent_engine(distributed_control &dc,
                            graph_type& graph,
  ...
  private:
    ...
    void internal_signal_broadcast(vertex_id_type gvid,
      ...
      for (size_t i = 0;i < rmi.numprocs(); ++i) {
        rmi.remote_call(i, &xasync_consistent_engine::internal_signal_gvid,
                        gvid, message);
      }
    ...
    void internal_stop() {
      for (procid_t i = 0;i < rmi.numprocs(); ++i) {
        rmi.remote_call(i, &xasync_consistent_engine::rpc_internal_stop);
      }
    }
    ...
    bool try_to_quit(size_t threadid,
      ...
      if (stat == sched_status::EMPTY || force_stop) {
        ...
        for (procid_t i = 0;i < rmi.dc().numprocs(); ++i) {
          rmi.remote_call(i, &xasync_consistent_engine::set_endgame_mode);
        } 
    ...
    void eval_sched_task(const lvid_type lvid,
      ...
      foreach(procid_t mirror, local_vertex.mirrors()) {
        gather_futures.push_back(
            object_fiber_remote_request(rmi, 
                                        mirror, 
                                        &xasync_consistent_engine::perform_gather, 
                                        vid,
                                        vprog));
      }
     ...
     /*foreach(procid_t mirror, local_vertex.mirrors()) {
       rmi.remote_call(mirror, 
                       &xasync_consistent_engine::perform_scatter, 
                       vid,
                       vprog,
                       local_vertex.data());
     }*/
